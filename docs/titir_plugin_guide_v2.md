...
**Project:** TiTir · **Author:** Bijoy · **Mascot:** PuFi · **Doc version:** v2
...
plugin — is here. (§21 adds a short, optional excerpt of the shell
specification purely for orientation — nothing in it is required
reading, and it introduces no new MANDATORY requirement beyond what's
...
If you'd like your icon (and, optionally, your own plugin's interior)
to visually align with TiTir's own look rather than clash with it,
`titir_visual_guide.md` (§22) is a separate, shared, non-enforced
reference — color tokens, the icon sizing math, and PuFi's mascot rules
— that both this guide and the technical specification point to. Using
it changes nothing about whether your plugin installs or mounts.
...
## 21. About the Shell — Context Only, Not Required Reading

Everything MANDATORY for building a plugin is already stated earlier in
this document — you do not need to read this section, and nothing here
adds a new requirement. It exists only because a plugin session is
never handed the technical specification alongside this guide, and a
short excerpt of what that document contains can make some of the rules
above (§4, §5, §9 especially) click faster if you're curious *why* they
exist.

The technical specification (`titir_technical_specification.md`) is the
shell-author-facing counterpart to this guide. In short, it describes:

- **What TiTir is, structurally.** One main process, one shell renderer
  (navigation column + idle state only, in React), and one
  `WebContentsView` per mounted plugin — including the built-in Hub,
  which is mounted like any other plugin, not baked into the shell
  renderer.
- **Why your view is sandboxed.** Because plugins are untrusted
  third-party code by definition, the shell deliberately keeps every
  plugin's preload as thin as possible — the three security flags in
  this guide's §4 (`contextIsolation`, `sandbox`, `nodeIntegration`)
  exist specifically so that a plugin's entire capability surface is
  exactly the IPC channel table this guide's §10 draws from, nothing
  more.
- **How install and mount are implemented on the shell's side** — the
  same path-safety check, manifest validation, and registration steps
  in this guide's §3 and §4, but from the perspective of the
  `plugin-installer.ts`/`plugin-loader.ts` code that runs them.
- **What's explicitly out of scope for v1** on the shell side —
  notably, a plugin marketplace, enforced permission gating, and any
  shared visual design system (see §22 below for the one narrow
  exception to that last point).
...
## 22. Visual & Presentation Guide — Shared, Optional Resource

A separate file, `titir_visual_guide.md`, holds the shared visual
reference — color tokens, type scale, the icon-legibility math behind
§15's sizing advice, and PuFi's mascot-usage rules. It is referenced
from both this guide and the technical specification so that the shell's
own chrome and any plugin author who wants to visually align with
TiTir draw from the same source instead of two documents drifting apart
independently.

Nothing in that file is enforced by the shell — see §15 above and §2 of
the technical specification. Use it only if visual alignment with
TiTir's own look is something you want for your plugin's icon or
interior; a plugin that ignores it entirely is exactly as legal as one
that doesn't.
